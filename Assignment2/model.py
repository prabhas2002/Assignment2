...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.lora_attn = LoRALinear(config.n_embd, 3 * config.n_embd, rank=config.LoRA_rank)
        # 2. add the auxiliary LoRA linear layer here corresponding to c_proj
        self.lora_proj = LoRALinear(config.n_embd, config.n_embd, rank=config.LoRA_rank)
        # 3. remove the gradients from the c_attn and c_proj layers
        for param in self.c_attn.parameters():
            param.requires_grad = False

        for param in self.c_proj.parameters():
            param.requires_grad = False
    ...
    def forward(self, x):
        ...
        lora_out = self.lora_attn(x)
        attn_out = attn_out + lora_out
        ...
        lora_proj_out = self.lora_proj(y)
        proj_out = proj_out + lora_proj_out
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.lora_fc = LoRALinear(config.n_embd, 4 * config.n_embd, rank=config.LoRA_rank)
        # 2. add the auxiliary LoRA linear layer here corresponding to c_proj
        self.lora_proj = LoRALinear(4 * config.n_embd, config.n_embd, rank=config.LoRA_rank)
        # 3. remove the gradients from the c_fc and c_proj layers
        for param in self.c_fc.parameters():
            param.requires_grad = False
...
class LoRALinear(nn.Module):
    ...
    def forward(self, x):
        # compute the LoRA decomposition
        x= self.dropout(x)
        x= x @ self.U @ self.V
...
class DistilRNN(nn.Module):
    def __init__(self):
        ...
        self.input_size = 768  #embed of GPT2
        self.hidden_size = 768
        self.output_size = 2   # binary classification
        self.num_layers = 1
        
        self.embed = nn.Embedding(50257, self.input_size)  # GPT2 vocab size
